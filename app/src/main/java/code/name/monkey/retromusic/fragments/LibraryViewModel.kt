...
class LibraryViewModel(
    ...
    fun getTotalPlaytime(songs: List<Song>): LiveData<Long> = liveData(IO) {
        emit(MusicUtil.getTotalPlaytimeDuration(songs))
    }

    private suspend fun fetchSongs() {
        songs.postValue(repository.songsWithPlayTime(repository.allSongs()))
    }
    ...
    fun recentSongs(): LiveData<List<Song>> = liveData(IO) {
        emit(repository.songsWithPlayTime(repository.recentSongs()))
    }
    ...
    fun observableHistorySongs(): LiveData<List<Song>> {
        viewModelScope.launch(IO) {
            ...
            songHistory.postValue(repository.songsWithPlayTime(repository.historySong().map {
                it.toSong()
            }))
        }
        return songHistory
    ...
    fun favorites(): LiveData<List<Song>> = repository.favorites().switchMap { songEntities ->
        liveData(IO) {
            emit(repository.songsWithPlayTime(songEntities.map { it.toSong() }))
        }
    }
