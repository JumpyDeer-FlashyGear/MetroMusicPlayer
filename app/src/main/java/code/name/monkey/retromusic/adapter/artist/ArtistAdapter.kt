...
open class ArtistAdapter(
    ...
    /**
     * Subtitle text shown under the artist name, or null/empty to hide the row entirely
     * (the default, matching every existing screen this adapter backs). Overridden by
     * [code.name.monkey.retromusic.adapter.artist.StatsArtistAdapter] to show a formatted
     * total playtime instead — see CLAUDE.md, Component 3.
     */
    protected open fun getArtistText(artist: Artist): String? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val artistText = getArtistText(artist)
        if (artistText.isNullOrEmpty()) {
            holder.text?.hide()
        } else {
            holder.text?.text = artistText
            holder.text?.show()
        }
        val transitionName =
