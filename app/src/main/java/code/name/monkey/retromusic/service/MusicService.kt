...
class MusicService : MediaBrowserServiceCompat(),
    ...
    private val repository: RealRepository = get(RealRepository::class.java)
    ...
    private var lastPlayedSongId: Long = -1L
    private var lastPlayedElapsedMillis: Long = 0L
    private var lastMetaChangeIsRealTransition: Boolean = false
    ...
    private fun saveSongPlayTime(songElapsedMillis: Long) {
        val previousSongId = songPlayCountHelper.song.id
    ...
    override fun onDestroy() {
        if (!songPlayCountHelper.isSongSaved) {
            val songID = songPlayCountHelper.song.title
            val songElapsedTime = songPlayCountHelper.songElapsedTime()
            Log.d("PlayTimeDebug", "onDestroy Added $songElapsedTime to $songID")
    ...
    override fun onTrackEnded() {
        ...
        if (!songPlayCountHelper.isSongSaved) {
            val songID = songPlayCountHelper.song.title
            val songElapsedTime = songPlayCountHelper.songElapsedTime()
            Log.d("PlayTimeDebug", "onTrackEnded Added $songElapsedTime to $songID")
            saveSongPlayTime(songElapsedTime)
        }
    ...
    fun openTrackAndPrepareNextAt(
        ...
        if (!songPlayCountHelper.isSongSaved && songPlayCountHelper.song.id != -1L) {
            val songID = songPlayCountHelper.song.title
            val songElapsedTime = songPlayCountHelper.songElapsedTime()
            Log.d("PlayTimeDebug", "openTrackAndPrepareNextAt Added $songElapsedTime to $songID")
            saveSongPlayTime(songElapsedTime)
        }
    ...
    fun updateMediaSessionPlaybackState() {
        if (!isPlaying && !songPlayCountHelper.isSongSaved && songPlayCountHelper.song.id != -1L){
            val songID = songPlayCountHelper.song.title
            val songElapsedTime = songPlayCountHelper.songElapsedTime()
            Log.d("PlayTimeDebug", "updateMediaSessionPlaybackState Added $songElapsedTime to $songID")
            saveSongPlayTime(songElapsedTime)
        }
        ...
        when (what) {
            ...
            META_CHANGED -> {
                ...
                serviceScope.launch(IO) {
                    ...
                    Log.d("PlayTimeDebug", "Calling notifySongChanged")
    ...
    private fun sendChangeInternal(what: String) {
        ...
        if (what == META_CHANGED && lastMetaChangeIsRealTransition) {
            intent.putExtra(EXTRA_PREVIOUS_SONG_ID, lastPlayedSongId)
            intent.putExtra(EXTRA_PREVIOUS_ELAPSED_MILLIS, lastPlayedElapsedMillis)
        }
    ...
    companion object {
        ...
        const val EXTRA_PREVIOUS_SONG_ID = "$RETRO_MUSIC_PACKAGE_NAME.extra.previous_song_id"
        const val EXTRA_PREVIOUS_ELAPSED_MILLIS = "$RETRO_MUSIC_PACKAGE_NAME.extra.previous_elapsed_millis"
        ...
        const val LYRICS_CHANGED = "$RETRO_MUSIC_PACKAGE_NAME.lyricschanged"
