...
// import code.name.monkey.retromusic.model.PlayTimeSong
...
object MusicUtil : KoinComponent {
    ...
    suspend fun getTotalPlaytimeDuration(songs: List<Song>): Long = withContext(IO) {
        val statsById = repository.playCountSongs().associateBy { it.id }
        songs.sumOf { song -> statsById[song.id]?.playTime ?: 0L }
    }
